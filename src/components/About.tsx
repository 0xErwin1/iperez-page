...
const About: React.FC = () => {
  return (
    <>
      <p>
        Currently, I work as a Full-Stack Web Developer with a focus more on
        backend using Node.js, Express, and Typescript. Although I have done
        some frontend work with React.js, it is not my strong suit. I have been
        using <label className="text-arch-color">{Icons.ArchLInux}</label> Arch
        Linux as my primary operating system for over 3 years. I am interested
        in all low-level systems and components, and in understanding how
        computers and programming languages work behind the scenes.
      </p>
        ...
        <div>
          ...
          <ul className="list list-inside grid grid-cols-2">
            {languageList.map((language) => (
              <li className="p-1">
                <label className={`${language.color} text-xl`}>
                  {language.icon}
                </label>{" "}
                {language.name}
              </li>
        ...
        <div>
          ...
          <ul className="list list-inside grid grid-cols-2">
            {frameworkList.map((framework) => (
              <li className="p-1">
                <label className={`${framework.color} text-xl`}>
                  {framework.icon}
                </label>{" "}
                {framework.name}
              </li>
          ...
          <ul className="list list-inside grid grid-cols-2">
            {databaseList.map((database) => (
              <li className="p-1">
                <label className={`${database.color} text-xl`}>
                  {database.icon}
                </label>{" "}
                {database.name}
              </li>
          ...
          <ul className="list list-inside grid grid-cols-2">
            {osList.map((os) => (
              <li className="p-1">
                <label className={`${os.color} text-xl`}>{os.icon}</label>{" "}
                {os.name}
              </li>
            ))}
          </ul>
      ...
      <p className="text-[10px] text-gray mt-5">
        **Obviously, I don't know these technologies perfectly; they are
        technologies that I believe I can work on a project with, encountering
        more or fewer problems. For more assurance, check out my GitHub.
      </p>
    </>
  );
};
